...
def go_target_position(shape):
    ...
    if shape == 1 or shape == 2 or shape == 3:
        target_position = small_positions[shape-1]
    if shape == 4 or shape == 5 or shape == 6:
        target_position = large_positions[shape-4]
    while action != 1: #after user do 1 action, the while loop will stop
        emg.update()#to keep obtain the lastest value from the sensor 
        time.sleep(0.25)
        emg.leftArm = arm.emg_left()
...
def main():
    ...
    for shape in range(1,7):
        ...
        #num = inventory.count(shape)
        #if num>1:
        #print("This container has already been placed!\nPlease select a different container!")
        #else:
